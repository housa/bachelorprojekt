...
namespace bp {
    ...
    void sha(std::vector<U32>& input, std::vector<U32>& dest, size_t input_size) {
        ...
        // Zero-padding
        while (bv_in.size() % libsnark::SHA256_block_size != 0 || bv_in.size() < input_size) {
            bv_in.push_back(0);
        }
        ...
        for (S32 i = libsnark::SHA256_block_size; i < bv_in.size(); i += chunk_size) {
            bv_tmp.clear();
